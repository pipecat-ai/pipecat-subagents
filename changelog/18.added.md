- **UI Agent protocol (v1).** First-class support for AI agents that observe
  ...
  - New `UIAgent` class (`LLMContextAgent` subclass) that dispatches client UI
    ...
    Task processing is single-flight: `on_task_request` acquires a
    per-agent lock that is held until `respond_to_task` fires, so
    overlapping requests queue rather than interleaving their context
    mutations.
  ...
  - New `attach_ui_bridge(root_agent)` that wires the new first-class
    UI RTVI channels to the agent bus in both directions: `ui-event`
